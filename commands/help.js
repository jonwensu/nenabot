exports.run = (client, message, args) => {
	...
	if (!searched) {
		message.channel.send(
			`Use \`${process.env.BOT_PREFIX}commands\` to view a list of commands. Use \`${process.env.BOT_PREFIX}help [command]\` to view more information on a specific command.`
		);
		return;
	}
	...
	if (!Object.keys(client.commands).includes(searched.toLowerCase())) {
		message.channel.send(
			`Sorry, but I don't think that command exists. Use \`${process.env.BOT_PREFIX}commands\` for a list of commands.`
		);
		return;
	}
	...
	if (!client.commands[searched].help) {
		message.channel.send(
			`Sorry, but I don't have information on that command.`
		);
		return;
	}
